...
public class DistributorDAO {
	...
	private static final String CREATE_DISTRIBUTOR = "insert into distributor (pd_name,contact,contact2,email,address,zone,city,"
			+ "state,lat,lng,enrolled_date,rating,description,commission) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private static final String UPDATE_DISTRIBUTOR = "update distributor set pd_name = ? , contact = ? , contact2 = ? , "
			+ "email = ? , address = ? , zone = ? ,city = ? , state = ? , lat = ? , lng = ? , enrolled_date = ? , rating = ? , "
	...
	public int createDistributor(DistributorDetailDTO distributorDetailDTO)
		...
		ResultSet rs = null;
		try {
			pstmt = conn.prepareStatement(CREATE_DISTRIBUTOR,
			...
			pstmt.setDate(11, distributorDetailDTO.getEnrolledDate());
			pstmt.setInt(12, distributorDetailDTO.getRating());
			pstmt.setString(13, distributorDetailDTO.getDescription());
			pstmt.setDouble(14, distributorDetailDTO.getCommission());
	...
	public void updateDistributor(DistributorDetailDTO distributorDetailDTO)
		...
		try {
			...
			pstmt.setString(13, distributorDetailDTO.getDescription());
			pstmt.setDouble(14, distributorDetailDTO.getCommission());
			pstmt.setLong(15, distributorDetailDTO.getDid());
