...
public class VendorDAO {
	...
	private static final String CREATE_VENDER = "insert into garage (g_name,contact,contact2,email,address,zone,city,state,lat,"
			+ "lng,enrolled_date,rating,description,commision_discuss) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private static final String UPDATE_VENDER = "update garage set g_name = ?,contact = ?,contact2 = ?,email = ? ,"
			+ "address = ?,zone = ? ,city = ?,state = ?,lat = ? ,lng = ?,enrolled_date = ?,rating = ? ,description = ?,"
	...
	public int createVendor(VendorDetailDTO vendorDetailDTO) {
		...
		try {
			...
			pstmt.setDate(11, vendorDetailDTO.getEnrolledDate());
			pstmt.setInt(12, vendorDetailDTO.getRating());
			pstmt.setString(13, vendorDetailDTO.getDescription());
			pstmt.setDouble(14, vendorDetailDTO.getCommission());
	...
	public void updateVendor(VendorDetailDTO vendorDetailDTO) {
		...
		try {
			...
			pstmt.setString(13, vendorDetailDTO.getDescription());
			pstmt.setDouble(14, vendorDetailDTO.getCommission());
			pstmt.setLong(15, vendorDetailDTO.getVid());
